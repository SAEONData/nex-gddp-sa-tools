...
def run(cfg):
    t0 = time.time()
    print("🧮 Starting TN10p (per-model cached thresholds, single-file inputs)…")
            ...
            for model, scen_file in sorted(model_files.items()):
                ...
                try:
                    with _open_single(scen_file) as ds, xr.open_dataset(cache_path, engine="h5netcdf") as thr_ds:
                        ...
                        below = (tn_scen_c < thr_t).chunk({"time": -1})  # daily bool, single time chunk
