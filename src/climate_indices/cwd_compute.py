...
cwd_compute.py
---------------------------------------------------------------
Compute CMIP6 CWD (maximum consecutive wet days) and save
...
def run(cfg):
    ...
    cwd_cfg    = cfg.get("cwd", {})
    threshold  = float(cwd_cfg.get("threshold_mm", 1.0))
    aggr       = cwd_cfg.get("aggregation", "annual")
    aggr_code  = cwd_cfg.get("aggregation_code", "YS")  # e.g. 'MS', 'QS-DEC', 'YS'
        ...
        for slice_name in slice_names:
            ...
            for i, nc_file in enumerate(nc_files, 1):
                ...
                try:
                    ...
                    cwd_result = CWD()(pr=pr, thresh=f"{threshold} mm/day", freq=aggr_code)
                    if cwd_result.size == 0:
                        raise ValueError("Empty result after resampling.")

                    # mean across aggregated periods in this slice (keep lazy)
            ...
            max_cwd = ensemble_mean.assign_attrs({
                "units": "days",
            ...
            ds_out = xr.Dataset(
                {"max_cwd": max_cwd},
                attrs={
                    "title": f"Ensemble Mean of CWD - {experiment} - {slice_name}",
                    "description": f"CWD with PR ≥ {threshold} mm/day; aggregation: {aggr} ({aggr_code})",
                    "models_included": ", ".join(sorted(set(model_names))),
                    "created_by": "cwd_compute.py",
                },
            )
