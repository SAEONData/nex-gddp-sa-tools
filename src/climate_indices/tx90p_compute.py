...
def _calendar90p_threshold(tas_hist_c: xr.DataArray) -> xr.DataArray:
    """TX90p (90th percentile by calendar day) on baseline."""
...
def run(cfg):
    t0 = time.time()
    print("🧮 Starting TX90p (per-model cached thresholds, streaming ensemble)…")
    ...
    # ---- 2) Per experiment & slice, streaming ensemble of % days > TX90p ----
        ...
        for slice_name in slice_names:
            ...
            for model in models:
                try:
                    ...
                    if (tx90p.sizes.get("lat") != tasmax.sizes.get("lat")
                        or tx90p.sizes.get("lon") != tasmax.sizes.get("lon")):
                        tx90p = tx90p.reindex(lat=tasmax["lat"], lon=tasmax["lon"], method=None)

                    doy = tasmax["time"].dt.dayofyear
                    max_doy = int(doy.max())
                    tx90p = _ensure_thr_has_day(tx90p, max_doy)
                    thr_t = tx90p.sel(dayofyear=doy)
