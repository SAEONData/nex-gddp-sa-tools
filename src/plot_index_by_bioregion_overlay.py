...
def _build_mask_from_gdf(gdf: gpd.GeoDataFrame, ref_da: xr.DataArray) -> xr.DataArray:
    """Return boolean mask aligned to ref_da grid (True = inside)."""
...
def plot_time_slices_by_bioregion_overlay(
    ...
    overlay_shapes = _to_epsg4326(gpd.read_file(shapefile_path))
    ...
    mask_cache = None
    mask_sig = None  # (shape, lon_name, lat_name)
    ...
    for scenario in scenario_order:
        for label in time_labels:
            ...
            if (mask_cache is None) or (sig != mask_sig):
                mask_cache = _build_mask_from_gdf(overlay_shapes, da)
                mask_sig = sig
    ...
    if last_mappable is not None:
        ...
        # (optional) tiny labels next to the lines
    ...
    # ---- Save/show ----
    if output_path:
        suffix = "anomalies_overlay.png" if anomaly else "ensemble_overlay.png"
